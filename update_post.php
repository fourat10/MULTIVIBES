...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_post'])) {
    ...
    $id = mysqli_real_escape_string($connection, $_GET['id']); // Use the ID from the GET parameter
        ...
        <form id="editPostForm" action="update_post.php?id=<?= htmlspecialchars($id) ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="category" id="category" required>
                    ...
                    <option value="1" <?= $row['category_id'] == 1 ? 'selected' : '' ?>>One</option>
                    <option value="2" <?= $row['category_id'] == 2 ? 'selected' : '' ?>>Two</option>
                    <option value="3" <?= $row['category_id'] == 3 ? 'selected' : '' ?>>Three</option>
                </select>
            </div>
